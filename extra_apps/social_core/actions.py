...
def do_complete(backend, login, user=None, redirect_name='next',
    ...
    # 用户名 和 token 过期时间一天
    response.set_cookie("name",value=user.name if user.name else user.username,max_age=3600*24)
    response.set_cookie("token",value=jwt_encode_handler(payload),max_age=3600*24)
    return response
